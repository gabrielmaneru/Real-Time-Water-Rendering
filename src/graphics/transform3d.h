...
struct transform3d
{
	...
	const float& get_scl()const { return m_tr.m_scl; }
	...
	void set_scl(const float& v) { m_tr.m_scl = v, m_tr.upd(); }
	void set_rot(const vec3& v) { m_tr.m_rot = v, m_tr.upd(); }
	void set_tr(const vec3& pos, const float& scl, const vec3& rot = vec3{ .0f, .0f, .0f })
	{
